...
public class AdminApi internal constructor(private val client: UarpClient) {
     ...
     * Required scopes: `admin`.
     */
    public suspend fun exportDataExplorer(namespace: String? = null, includeSensitive: ExportDataExplorerIncludeSensitive? = null, options: RequestOptions = RequestOptions()): String {
        val query = buildList {
            if (namespace != null) add("namespace" to namespace)
            if (includeSensitive != null) add("include_sensitive" to includeSensitive.value)
        }
     ...
     * Required scopes: `admin`.
     */
    public suspend fun getPlatformEconomics(refresh: ExportDataExplorerIncludeSensitive? = null, options: RequestOptions = RequestOptions()): PlatformEconomics {
        val query = buildList {
     ...
     * Required scopes: `admin`.
     */
    public suspend fun importDataExplorer(body: ImportDataExplorerRequest, options: RequestOptions = RequestOptions()): ImportDataExplorerResponse {
        val parts = buildList {
            add(Part.File("file", body.`file`))
        }
        return client.request<ImportDataExplorerResponse>(
            RequestSpec(
                method = "POST",
     ...
     * `/admin&#47;*` path 404s instead of falling through to this feed. Requires the `admin` role
     * and super-admin identity, as every `/admin` route does.
     *
