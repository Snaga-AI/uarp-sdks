...
package UARP.API.Workspaces is
   ...
   --  (JWT auth) the file is moved to `.trash/console/...` with a manifest row and the response
   --  says `trashed: true` with the trash path, so it can be restored; `?trash=false` forces a
